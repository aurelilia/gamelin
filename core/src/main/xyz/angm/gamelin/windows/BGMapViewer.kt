 ...
 * This file was last modified at 3/18/21, 1:48 PM.
...
class BGMapViewer(private val gb: GameBoy) : Window("BG Map Viewer") {
    ...
    private val bgRender = TileRenderer(gb, 32, 32, 2f)
    private val windowRender = TileRenderer(gb, 32, 32, 2f)
    ...
    private fun draw(renderer: TileRenderer, window: Boolean) {
        for (tileIdxAddr in 0 until 32 * 32) {
            val dataAddr = gb.ppu.bgIdxTileDataAddr(window, tileIdxAddr)
